/*
	Project: JsExp
	File: js/customMultipleSelect_checkbox.js
...
var obj = {
    ...
    buildCustomSelect: function () {// builds the pseudo select box
        ...
        for (var i = 0; i < this.selectTexts.length; i++) {
            this.restWave.push('<li><input type="checkbox" name=	"' + this.selectTexts[i] + '" id="' + i + '"/><label for="' + i + '"/>' + this.selectTexts[i] + '</label></li>');
        }
        this.endList = '</ul></div></div><div id="msg"></div><div class="infoMsgs"></div></div>';
    ...
    doHere: function () {
        ...
        this.checkbox = $(".restAll input:checkbox");
        for (var i = 0; i < this.checkbox.length; i++) {
            $(this.checkbox[i]).bind('change', (function (i) {
                return function () {
                    if (this.checked == true) {
						//$($(".restAll input:checkbox")[i]).checked = false;
                        this.optVal = $($(".restAll input:checkbox")[i]).attr('id');
                        ...
                        $(".selectCustom>div.opt").live('click', function () {
                            ...
                            $($(".restAll input:checkbox")[this.sp]).attr('checked', false);
